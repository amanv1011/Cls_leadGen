...
export const filterLeads = (campaignIds, leadsList, searchDate, searchQuery) => {
 ...
 if(searchDate !== ""){
   ...
   for (let i = 0; i < campaignIds.length; i++) {
     for (let j = 0; j < leadsList.length; j++) {
       ...
       const between =  moment.unix(leadsList[i].leadGeneratedDate.seconds).format("YYYY-MM-DD")
       ...
       if ((leadsList[j].campaignId === campaignIds[i].id) && (unixTimestampStart < unixTimestampBetween && unixTimestampBetween < unixTimestampEnd)) {
        filteredResults.push(leadsList[j]);
       }
     }
   }
